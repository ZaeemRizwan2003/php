...
$rows = $con->query("SELECT * FROM the_hotel_picture WHERE hotel_id = '$id'")->num_rows;
?>
        ...
        <form action="hotel-images-list?id=<?= $id ?>" method="POST">
            <div class="card">
                <div class="card-header" style="display: block;">
                        ...
                        if ($_POST) {
                            if (isset($_POST['tumresimler'])) {
                                ...
                                foreach ($resimler as $resim) {
                                    $bul = $con->query("SELECT * FROM the_hotel_picture WHERE picture_id = '$resim'")->fetch_object();
                                    unlink('../data/hotel/pictures/' . $bul->mini_picture);
                                    unlink('../data/hotel/pictures/' . $bul->big_picture);
                                    $sil = $con->query("DELETE FROM the_hotel_picture WHERE picture_id = '$resim'");
                                }
                                if ($sil) {
                    ...
                    <a href="hotel-images-add?id=<?= $id ?>" class="btn btn-success"> <i class="fe fe-plus"></i> Resim
                        Ekle </a>
                    <?php if ($rows) { ?>
                        <button type="submit" class="btn btn-danger pull-right mr-4"> <i class="fe fe-trash"></i> Seçilen
                            Resimleri Sil</button>
                ...
                <div class="card-body">
                    <table class="table card-table table-vcenter text-nowrap" id="koby_table">
                        ...
                        <tbody>
                            ...
                            <?php
                            $list = $con->query("SELECT * FROM the_hotel_picture WHERE hotel_id = '$id' ORDER BY rank ASC");
                                ...
                                <tr>
                                    ...
                                    <td id="<?= $view->sira ?>">
                                        ...
                                        <input type="number" class="form-control" style="width: 70px"
                                            onchange="rankChange(this,'?do=hotel&q=pic-rank')" value="<?= $view->rank ?>"
                                            name="rank" id="<?= $view->picture_id ?>">
                                    </td>
                                    <th>
                                        <div class="none"><?= $view->mini_picture ?></div>
                                        <a href="#">
                                            <img src="../data/hotel/pictures/<?= $view->mini_picture ?>"
                                    ...
                                    <th>
                                        ...
                                        <textarea name="content" class="form-control"
                                            onchange="contentChange(this,'?do=hotel&q=pic-content')"
                                            id="<?= $view->picture_id ?>" cols="30" rows="3"><?= $view->content ?></textarea>
                                    </th>
                                    <th class="text-center">
                                        <a href="javascript:void(0)"
                                            onclick="kobySingle('<?= $view->hotel_id ?>','?do=hotel&q=pic-delete&id=<?= $view->picture_id ?>','hotel-images-list?id=<?= $id ?>')"
